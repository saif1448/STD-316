...
class Inventory:
    ...
    def __init__(self) -> None:
        ...
        self._products = [] # list of Product objects, List[Product]
    ...
    def _find_product(self, name: str) -> Product:
        ...
        """
        # laptop = Product("LAPTOP", 2030.5, 2)
        # mouse = Product("MOUSE", 25.0, 5)
        # keyboard = Product("keyboard", 45.0, 3)
        # retail_store_inventory._products = [laptop, mouse]
        # retail_store_inventory._find_product("MOUSE") -> laptop
    ...
    def stock_product(self, product: Product) -> None:
        ...
        # retail_store_inventory.stock_product(laptop)
        # mouse = Product("mouse", 25.0, 5)
        # retail_store_inventory.stock_product(mouse)
        # [laptop, mouse] -> retail_store_inventory._products
        # laptop2 = Product("laptop", 2030.5, 3)
        # retail_store_inventory.stock_product(laptop2)

        assert isinstance(product, Product), "the parameter product must be an instance of the Class Product"

        # this is for the exising product
        new_product_name = product.get_name()
        if self._find_product(new_product_name):
            existing_product = self._find_product(new_product_name)
            new_quantity = product.get_quantity()
            existing_product.add_quantity(new_quantity)
        else:
